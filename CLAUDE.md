...
- **Verificare E2E completa toate fluxurile (2026-07-12, commits `4371d61`→`321b845`):** COMPLETATA — testare directa pe serviciul LIVE (nu scripturi izolate), toate 9 AnalysisType + 8 formate raport + WS + monitoring + batch + compare + companii + NLQ + quick-tools + OCR + settings, cu efecte reale aprobate (Telegram/email). Plan complet: `99_Plan_vs_Audit/PLAN_E2E_VERIFICARE_2026-07-12.md`. **6 bug-uri reale gasite + reparate:** (1) Cerebras — model `qwen-3-235b` retras din catalogul vendorului, mort ~2-3 luni (HTTP 404 pe fiecare apel) → migrat la `gpt-oss-120b`; (2) `companies.caen_code`/`county` niciodata populate (INSERT original nu le seta) → `/sector`, `compare/sector`, filtrul `?caen=` erau goale silentios pt orice firma → fix + backfill 7 firme; (3) `score-trend` — `company_id: int` cand schema e UUID TEXT → HTTP 422 garantat, feature mort de la introducere → `str`; (4) `timeline-report/pdf` — crash 100% reproductibil (`FPDFUnicodeEncodingException` pe em-dash in header, ocolea helper-ul `_sanitize()` local) → fix; (5) `export/ics` — citea cheia `market.seap_tenders` (niciodata scrisa) in loc de `tender_opportunities.opportunities` (Angle A v2 real) → mort de la introducere → fix; (6) `settings test/{service}` — Mistral+Cerebras (2/5 provideri sinteza) netestabile, exact golul care a lasat bug #1 nedetectat → adaugate; (7, gasit prin verificare adversariala advisor) sectiunea narativa "opportunities" (AnalysisType TENDER_OPPORTUNITIES) nu citea `tender_opportunities` (Angle A v2) — cadea mereu pe fallback chiar cu 15 licitatii reale disponibile → fix gate + context prompt, verificat live (350→3080 caractere text real). **Descoperire conexa NEreparata:** `COMPETITION_ANALYSIS` nu are nicio colectare reala de date despre competitori (camp `web_presence.competitors` nescris nicaieri) — feature lipsa, nu bug de wiring, semnalat userului. **Onestitate scope:** doar 3/9 AnalysisType testate live (FULL_COMPANY_PROFILE, PARTNER_RISK_ASSESSMENT, TENDER_OPPORTUNITIES) — celelalte 6 pot avea bug-uri proprii netestate; degradarea sub sarcina concurenta e mult redusa dar NU eliminata (retest 2 joburi: 1/10 sectiuni tot a picat); verificare doar la nivel API, nu si vizual in browser. **SYNTHESIS_MODE=autonomous investigat, NU comutat:** serviciul ruleaza ca SYSTEM, Claude CLI auth exista doar in profilul userului → switch la `claude_code` fara schimbare de cont serviciu ar cadea tacut pe fallback (confirmat empiric: `Not logged in`, 6.3s) — 3 optiuni documentate, decizie lasata userului. 440 pytest PASSED, 0 erori TypeScript, toate fix-urile verificate live (nu doar cod).
...
- **49+ REST endpoints + 1 WebSocket + 16 pagini frontend + 8 formate raport (cu câmpuri bogate randate) + diagnostic + audit + request tracing + notifications + favorites + timeline + OCR UI + /metrics**
