...
- **Audit complet 88/88 endpoint-uri + 15/15 pagini verificate vizual (2026-07-13):** userul a cerut citirea completa a ce a ramas netestat din `AUDIT_FUNCTII.html` + continuarea executiei. **74/88 endpoint-uri** (de la 56/88) — toate cele 18 GET-uri sigure/fara efecte secundare testate live (analysis/types, companies/{id}, compare/sector/dashboard, jobs, notifications, reports/data, settings, cache/stats, health, health/deep, stats, stats/trend, metrics). **GASIT:** `/metrics` raspunde 200 dar cu `{"error":"prometheus-client not installed"}` — endpoint neconectat real. **14 ramase netestate DELIBERAT** (efecte reale/disruptive — companies/import, settings PUT, restart, update, jobs/cancel, etc.) — necesita decizie explicita inainte de a le rula. **15/15 pagini frontend verificate vizual in browser real** (Chrome, nu doar curl) — **2 bug-uri reale gasite + reparate**: (1) badge scor risc pe `/companies` afisa mereu "N/A" — citea camp inexistent `last_score` (corect: `last_risk_score_numeric`), care nici nu era in `COMPANY_COLS`; (2) pe `/company/:id`, cardurile "Analize"/"Prima Analiza"/descriere CAEN erau goale — `COMPANY_COLS` (single source of truth din `backend/routers/companies.py`) nu includea `analysis_count`, `first_analyzed_at`, `caen_description`, `city` desi existau populate real in DB de multa vreme. Reparat ambele (COMPANY_COLS extins + `frontend/src/lib/types.ts` Company interface + `Companies.tsx` simplificat, fara type-cast hack). **Gotcha reconfirmat:** service worker-ul PWA serveste bundle vechi dupa rebuild — hard-refresh sau tab nou necesar pt verificare vizuala reala. 443 pytest, tsc clean. Memory: `project_ris_audit_dashboard_ping_registry`.
