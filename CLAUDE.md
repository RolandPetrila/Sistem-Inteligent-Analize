...
- **Grup DRY #2 (get_risk_color) — INVENTAR complet, STOP inainte de editare (2026-07-14):** cautare exhaustiva (nu doar grep 70/40 literal) — 25 situri gasite (11 backend + 14 frontend) care converteg scor numeric in culoare/eticheta pt risc. Engine canonic: `scoring.py:1130-1135` (`>=70 Verde`, `>=40 Galben`, altfel Rosu). Descoperire: `scoring.py:145-148` are deja un `COLOR_MAP` EXPORTAT dar folosit NICAIERI, nici de engine-ul insusi (re-hardcodeaza inline) — "sursa unica" presupusa e de fapt decorativa. **Rezultat surprinzator:** ZERO divergenta de prag/operator gasita — toate 25 situri folosesc exact `>=70`/`>=40`, niciun `>70`/`>40` gresit la granita. Divergenta reala e de REPREZENTARE (7 mecanisme diferite: cuvant RO, hex crud, PatternFill openpyxl, RGBColor pptx, clasa Tailwind custom `risk-verde`, clase Tailwind generice `-400` vs `-500` — ultimele 2 vizual diferite desi acelasi prag, `ReportView.tsx` randeaza ambele nuante pe aceeasi pagina prin `<ReportHeader>`+cod propriu) + 3 cazuri de duplicare interna in acelasi fisier (`excel_generator.py`, `CompareCompanies.tsx`, `ReportView.tsx`+`ReportHeader.tsx`). Finding adiacent: `routers/analysis.py` quick-score foloseste o formula complet diferita (euristica ad-hoc) dar acelasi vocabular Verde/Galben/Rosu — poate induce confuzie ca ar fi acelasi calcul. NU s-a editat nimic — asteapta decizie Opus+Roland pt extractia reala (`get_risk_color` central + `RISK_COLOR_CONFIG` frontend).
...
- **Git:** https://github.com/RolandPetrila/Sistem-Inteligent-Analize.git | 467 pytest + vitest
