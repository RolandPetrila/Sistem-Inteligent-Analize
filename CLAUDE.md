...
- **Refactor #1 (AgentOfficial.execute) — Pas 0 + Faza A (2026-07-14, commits `26e379e`, `0aaccbd`):** PLAN scris + APROBAT de Opus inainte de orice cod (harta reala ~20 blocuri secventiale vs 6 presupuse de audit, consumatori verificati direct — `agent_verification.py` primeste `official_data` intreg la ~15 functii interne). **Pas 0:** test de caracterizare (`tests/test_agent_official_characterization.py`) — golden snapshot pe INTREG dict-ul de retur (nu doar scorul), 6 fixture-uri (2 branch-uri structurale: CUI invalid MOD11/fara CUI + 4 combinatii de surse: toate OK, ANAF pica/restul OK — garda CRITICA #2, toate pica, mix realist). Mock la seam-ul `fetch_with_retry` (dispecerizat dupa source_name) + `_fetch_aegrm` separat + toate importurile locale mockuite la modulul sursa + `datetime.now()` inghetat. **Dovada de non-vacuitate:** re-cuplat temporar regresia CRITICA #2 → testul a picat pe 2 fixture-uri (`anaf_fails_rest_succeed` + `all_sources_fail`) → revertit complet (git diff gol, verificat). **Descoperire adiacenta:** `tavily_quota_ok` gateaza SI legal-merged SI OSINT historical-flags (acelasi flag) — quota epuizata pierde silentios si semnalele OSINT, nu doar BPI/litigii (finding semnalat, NU reparat). **Faza A:** extrase 7 functii pure per-sursa (`_process_anaf_result`, `_process_openapi_result`, `_process_bilant_result`, `_process_bnr_result`, `_process_bpi_result`, `_process_aegrm_result` — pastreaza absenta din `sources`, `_derive_anaf_fiscal_risk`) — zero reordonare, golden IDENTIC. Faza B/C/D (store_administrators, Portal Just, Google Maps, MO Partea IV, fallback-uri, agregate) raman inline — STOP dupa Faza A, cum a cerut Opus, asteapta GO. 490 pytest PASSED (483+7).
...
- **Git:** https://github.com/RolandPetrila/Sistem-Inteligent-Analize.git | 490 pytest + vitest
