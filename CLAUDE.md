...
- **Sesiune 2026-07-25 (CERINTA #1 — reconfigurare provideri AI, 3 commits `a20164e`→`917d0ee`, LOCALE NEPUSHATE, mod jurnal AUDITOR↔EXECUTOR + Opus-advisor):** durabilitate providerilor AI. Coordonare prin `99_Plan_vs_Audit/JURNAL_AUDIT.md` (UNTRACKED — auditor scrie cerinte, executor raporteaza sub ele). **NOU `backend/agents/ai_models.py`** = sursa UNICA a numelor de model (erau hardcodate in 3 locuri) + `QUALITY_CHAIN`/`SPEED_CHAIN` (fallback SECVENTIAL ordonat — vechiul `_concurrent_fallback` FIRST_COMPLETED facea ordinea fictiune, Gemini castiga mereu, DeepSeek nu era atins NICIODATA) + 3 garzi: **§3** model retras (404/model_not_found→INDISPONIBIL pe sesiune, nu reapelat), **§4** context (skip la >90% + overflow runtime), **§5** cota (429→log distinct+retry-after, FARA circuit-trip). **§0 verificat live** GET /v1/models: **3 modele erau MOARTE in productie** — groq `llama-4-scout` (404 tacut la fiecare sectiune scurta), openrouter `deepseek-r1:free`, sambanova `405B` (retras→substituit `Meta-Llama-3.3-70B`, opt. auditor). **Ipoteza consultant „Cerebras 8192 cap" INFIRMATA empiric** (17k tokeni acceptati, 200) → cerebras=128k+overflow-runtime. **3 cai rupte reparate** de scout (synthesis, `lead_search`, `companies` chat + bug adiacent `settings.gemini_api_key` inexistent→`google_ai_api_key`). **NOU `tools/check_ai_models.py`** (§6 test lunar catalog). **E7 LIVE PASS** (job `bd69a5d7` CUI 26313362, DONE 8/8): Claude scrie exec_summary+financial (provider=claude, fara FALLBACK); pe timeout Claude → openrouter/deepseek (NU Gemini tacut — ordinea reala LIVE); groq 429 → §5 COTA EPUIZATA logat. **922 pytest** (+26, non-vacuitate §3/§4/§5 dovedita mecanic git-show-HEAD), ruff clean. **Finding:** groq free-tier 6000 tok/MIN (nu req/zi) → cerebras de-facto primar viteza. **Ramas:** audit executie (auditorul, dupa ancora `>>> EXECUTOR: CERINTA #1 FINALIZATA <<<`) + decizie push Roland.
- **Ciclurile AUDITOR↔EXECUTOR #2–#15 (2026-07-26 → 2026-07-29, mod jurnal serializat, TOATE PUSHATE pe origin/main):** continuare a modului din CERINTA #1 — auditorul scrie cerinta in `99_Plan_vs_Audit/JURNAL_AUDIT.md` (UNTRACKED, repo public), executorul livreaza sub ea cu non-vacuitate mecanica + job live, proprietarul pushuieste dupa verdict OK. Detaliu per-ciclu = JURNAL + memorie (`project_ris_*`). Rezumat: **#2–#11** = durabilitate provideri AI + doc-drift (RNPM `co.rnpm.ro` viu vs AEGRM mort, izolare chei platite in teste `conftest`, SambaNova scos 402, marker `payment_method_required`, corectie „serviciul=SYSTEM" FALS→`.\ALIENWARE` + `.env` bate env var, INS TEMPO viu pe INT101W). **#12–#15** = remediere audit al unei analize REALE (`raport (23).html`, MOSSLEIN 26313362) din `PLAN_REMEDIERE_RAPORT_2026-07-28.md`: **#12** wizard purpose/focus/period cablate in sinteza (`1da5d0c`); **#13** sectiunea narativa „Oportunitati" constienta de propriile date (`71d1a44`); **#14** perioade financiare personalizate `period_custom` (`123b97e`); **#15** surface onest fiabilitate SEAP — `seap_*_caveat` in HTML/PDF/DOCX cand numarul/valoarea sunt partiale (`518564a`). **RE-AUDIT OFICIAL PASS 5/5 (2026-07-29):** proprietarul a re-rulat MOSSLEIN (focus licitatii, nivel 3, Ultimii-5-ani); toate P1–P4 confirmate corect in livrabilul real vs baseline (Oportunitati numeste licitatii reale, financiar 5 ani, focus reflectat, ranking relevant, calificative SEAP prezente). **1040 pytest.** Slabiciuni masurate → CERINTA #16 (sinteza/SEAP W1–W3 + doc-currency).
- **CERINTA #16 (2026-07-30, imbunatatiri sinteza/SEAP W1–W3 + doc-currency, mod jurnal AUDITOR↔EXECUTOR):** din slabiciunile masurate la re-audit. **A (timeout):** `synthesis_claude_timeout` 360→480 + `synthesis_total_timeout` 2400→3600 (`config.py` + `.env` — `.env` bate config in RIS); job real: exec_summary rula 379s si era TAIAT la vechiul 360s → fallback openrouter cu context redus; acum termina pe Claude. **B (routing):** `opportunities` mutat pe QUALITY chain (`section_prompts.py`, ca `lead_candidates`) — sectiunea-cheie a proprietarului (focus licitatii) mergea pe cerebras (SPEED), acum Claude. **C (paginare SEAP):** `_fetch_seap_pages` pagineaza `pageIndex` pana la `total` real (era doar pagina 0 = 200 itemi) — MOSSLEIN avea 485 achizitii directe trunchiate la 200 → `items_truncated=True` desi 485<plafon 2000; acum aduce toate 485 → `counts_reliable=True`. Canar dedup pe id stabil (SICAP ignora tacit params → anti-inflatie). Ipoteza pageIndex CONFIRMATA LIVE (page0/page1 overlap=0). NU rupe #15: la plafon server `total_capped` ramane True → calificativ pastrat. **D:** `_soft_wrap_long_words` in PDF (vechiul break arunca tot dupa char 110 → pierdere continut + `[paragraf nerandat]`). **E (doc-currency):** regula HARD standing in CLAUDE.md + garda `tools/check_doc_drift.py` (functie pura + CLI, non-fatal in RIS_TEST.bat) + acest doc-update. **Job live MOSSLEIN `e601c6a4` DONE 1556s (<3600):** exec_summary+financial+risk+opportunities+recommendations toate `provider=claude` (fara FALLBACK), SEAP `items_truncated=False` (485 aduse), 526 contracte fiabile, value-caveat #15 corect PREZENT (acord-cadru value_unknown), count-caveat ABSENT. **1059 pytest** (+19, non-vac B/C/D mecanic git-show-HEAD), ruff clean, frontend NEATINS. Local, nepushat (push=proprietarul dupa audit).
- **CERINTA #17 (2026-07-30, P6 — omite sectiunea Competitie filler „date insuficiente", mod jurnal AUDITOR↔EXECUTOR):** cand `_has_sufficient_data` e False, `agent_synthesis.generate_section` intoarce un FILLER determinist („Sectiunea ... nu a putut fi generata din cauza datelor insuficiente...", `word_count=0`) care era STOCAT si RANDAT ca sectiune — masurat: `competition` cadea pe filler in 37/40 rapoarte (colectarea de competitori rar populata). **Fix = mark-and-skip la randare** (NU don't-emit — `test_synthesis_partial_preservation` cere ca `execute()` sa intoarca MEREU toate cheile de sectiune): sinteza marcheaza fillerul cu `INSUFFICIENT_DATA_MARKER=True` (`insufficient_data_filler`, nume distinct de cheia bool omonima din `due_diligence` checklist) la PUNCTUL UNIC de emisie; cheia RAMANE in `report_sections` (deci in DB/regenerare/chat), iar cei 4 randere (HTML/PDF/DOCX/PPTX) o OMIT prin `backend/reports/section_visibility.py::visible_sections`. **Invarianti:** #2 zero nav/anchor/TOC mort (nav HTML + `start_section` PDF sunt IN bucla, TOC DOCX = camp Word); #3 PPTX filtreaza INAINTE de slice `[:4]` (decizie declarata); #4 never-empty (daca TOATE sectiunile-s filler, se pastreaza — fillerul e atunci raspunsul onest, ex. COMPETITION_ANALYSIS gol). Excel/one_pager NU itereaza `report_sections` (neatinse). `_pdf_sanitize` (generator.py) e structure-preserving → markerul (cheie ASCII, valoare bool) supravietuieste sanitizarii latin-1 (verificat + LIVE pe raport.pdf real). **Non-vac MECANIC** (swap git-show-HEAD → teste portante PICA: absenta HTML/PDF/DOCX + marker producator + **PPTX filter-before-slice** cu fillerul pus la slot 2 si a 5-a sectiune promovata, vs HEAD~1; restore sha256 identic). **Job live MOSSLEIN `5a432e0e` FULL_COMPANY_PROFILE nivel 1 DONE 790s:** DB `competition.insufficient_data_filler=True` (cheie RETINUTA, doar competition marcat), disk HTML/PDF/DOCX omit „Analiza Competitie" + zero `#competition` anchor, exec_summary present (discriminant); PPTX omite si el, dar in acest job competition era pe slot 5 → absenta PPTX e pozitional NON-discriminanta (inv.#3 dovedit de test dedicat, nu de job). **Regenerare (inv.#6) DOVEDIT LIVE:** `POST .../section/competition/regenerate` (tot insuficient) → markerul supravietuieste UPDATE-ului DB (`insufficient_data_filler` ramane True). **1076 pytest** (+17), ruff clean, **frontend NEATINS** (ReportView live ramane consumator neschimbat — CANDIDAT separat). Lasat neschimbat notat: chat RAG + key_takeaways source includ fillerul ca azi (benign). `_has_sufficient_data` (pragul) NEATINS. Local, nepushat (push=proprietarul dupa audit).
- **CERINTA #18 (2026-07-30, P6-live — `ReportView` omite fillerul „date insuficiente" din vizualizarea LIVE, mod jurnal AUDITOR↔EXECUTOR):** continuarea logica a #17 (care reparase DOAR disk-ul HTML/PDF/DOCX/PPTX si lasase EXPLICIT `ReportView` ca „CANDIDAT separat"). Vederea live din PWA inca randa cardul „Analiza Competitie: date insuficiente" pe care raportul descarcat NU-l contine → livrabilul si vederea live se contraziceau. **Fix = oglinda TS a backend-ului** (NU re-calcul de suficienta pe frontend — citeste DOAR markerul deja pus): NOU `frontend/src/lib/section-visibility.ts` (`INSUFFICIENT_DATA_MARKER="insufficient_data_filler"` string EXACT ca backend, `isFillerSection`/`partitionSections`/`visibleSections`, never-empty inv.#4 intr-un singur loc). **UX (b) decis de proprietar:** fillerele NU apar in lista narativa, ci ca un **rand mut sub lista** („N sectiune(i) omisa(e) din raport (date insuficiente): …") care PASTREAZA butonul Regenereaza (zero pierdere de functionalitate). `ReportView.tsx`: `visible.map` pt carduri + rand mut pt `hidden`; tip `ReportSectionValue` + `api.regenerateSection.section` extinse cu markerul. **Corectie la cerinta (D1):** `ReportView` foloseste `api.getReport`/`GET /reports/{id}` (`get_report_by_id`), NU `getReportData`/`/data` (citat de auditor) — AMBELE pass-through raw `json.loads(full_data)` → invariantul se tine pe endpoint-ul REAL. **Anti-phantom pe payload VIU** (`GET /api/reports/fd0ce818-…`: `competition.insufficient_data_filler===True` ajunge la client). **Inv.#4 verificat pe RASPUNS nu doar DB** (`jobs.py:395-400` intoarce `new_section` VERBATIM cu marker din `generate_section:248-264` — linia CLAUDE.md TASK-1 „{title,content,word_count}" e STALE pt acest path → zero deviatie backend). **Non-vac MECANIC** (mut helper deoparte → vitest PICA „import lipsa" → restaurat sha256 identic; 10 teste noi). **LIVE MOSSLEIN `fd0ce818` (rebuild→restart→TAB NOU):** accesibility-tree = 6 sectiuni reale ca CARDURI + Analiza Competitie DOAR in rand mut (corpul fillerului nerandat); **inv.#4 LIVE** click Regenereaza pe competition → structura IDENTICA, ramane ascuns fara reload (marker supravietuieste in raspuns); **ZERO erori consola** (CSP/401). **105 vitest** (+10) **+ 1076 pytest** (backend NEATINS, garda), `tsc`+build+ruff clean. **NU ATINGE respectat** (backend #17, `get_report_by_id`, `REGENERATABLE_SECTIONS`, richFields, scoring/ai_models, `_has_sufficient_data`). Devieri: D1 (endpoint real), D2 (api.ts marker = mecanismul inv.#4). Lacune declarate: „devine suficient"/never-empty ne-producibile live (unit-test); drift cross-limbaj ne-pazit (out-of-scope). Commit `1936daa`, local nepushat (push=proprietarul dupa audit). Cu #18, si suprafata LIVE a raportului e consistenta cu livrabilul de pe disc.
...
- `backend/reports/section_visibility.py` — CERINTA #17: `visible_sections()` omite din raport sectiunile filler „date insuficiente" (marcate `INSUFFICIENT_DATA_MARKER` de sinteza), cu garda never-empty; folosit de cei 4 randere care itereaza `report_sections`
...
- `frontend/src/lib/section-visibility.ts` — CERINTA #18: oglinda TS a `backend/reports/section_visibility.py`; `isFillerSection`/`partitionSections`/`visibleSections` — `ReportView` omite din vizualizarea LIVE sectiunile filler „date insuficiente" (marker `insufficient_data_filler`), cu rand mut + Regenereaza (UX b) si never-empty
...
- Synthesis: subprocess `claude --print --model claude-opus-4-8 --effort {SYNTHESIS_EFFORT}` (default max), timeout `SYNTHESIS_CLAUDE_TIMEOUT` (**480s** din CERINTA #16, era 360 — coada Claude depasea 360s pe exec_summary/risk_assessment), plafon global `SYNTHESIS_TOTAL_TIMEOUT` (**3600s**, era 2400), fara `ANTHROPIC_API_KEY` in mediu ($0 Max)
...
| `DIRECTIVA_PERMANENTA_CERINTE.md` | Directiva permanenta a proprietarului (exhaustivitate inainte de orice cerinta de executie). Exceptie explicita user 2026-07-29. |
...
2. Groq (Llama 3.1 8B Instant, ex-Llama 4 Scout retras 2026-07) ca prim pas pe ruta viteza (gratuit). NB: free-tier 6000 tokens/min -> prompturile RIS il 429-esc frecvent -> cerebras devine de-facto primar viteza (degradare logata §5)
...
**REGULA HARD — DOC-CURRENCY (directiva proprietar 2026-07-30, oglindeste memoria
`feedback_doc_currency_mandatory`):** `CLAUDE.md` + docurile-cheie de mai sus se actualizeaza
**SI se commit** ca parte din FIECARE ciclu de lucru care schimba cod/comportament — nu ca pas
optional la final. **NU se lasa `M CLAUDE.md` (sau orice doc) necomis la predare/finalul sesiunii.**
Statusul se scrie cu judecata (R3) — NU un generator automat de proza; „auto-update autonom" =
aceasta disciplina standing + garda de doc-drift `tools/check_doc_drift.py` (avertizeaza cand
s-a schimbat cod fara actualizare doc in acelasi ciclu), NU un editor automat de CLAUDE.md.
_(Nota: in modul AUDITOR↔EXECUTOR, commit-ul de doc face parte din ciclu; **push-ul** ramane decizia
proprietarului dupa verdict — vezi ROL_EXECUTOR. Regula „Regula Commit Obligatoriu" de mai jos e
regula de baza a proiectului local.)_
