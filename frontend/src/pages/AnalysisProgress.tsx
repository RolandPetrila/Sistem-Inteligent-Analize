...
export default function AnalysisProgress() {
  ...
  const [logs, setLogs] = useState<
    { text: string; type: string; ts: string }[]
  >([]);
  ...
  useEffect(() => {
    ...
    api
      .getJob(id)
      .then(setJob)
      .catch(() => toast("Eroare la incarcarea jobului", "error"));
  }, [id]);
  ...
  const onMessage = (msg: WSMessage) => {
    ...
    if (msg.type === "progress" && msg.step) {
      setLogs((prev) => {
        ...
        if (prev.length > 0 && prev[prev.length - 1].text === msg.step)
          return prev;
    ...
    if (msg.type === "job_complete") {
      ...
      logAction("AnalysisProgress", "complete", {
        jobId: id,
      ...
      setLogs((prev) => [
        ...prev,
        {
          text: `Analiza finalizata! Formate: ${(msg.formats || []).join(", ").toUpperCase() || "N/A"}`,
          type: "success",
          ts: nowTs(),
        },
      ]);
  ...
  const statusIcon = {
    ...
    RUNNING: <Loader2 className="w-6 h-6 text-accent-primary animate-spin" />,
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      ...
      <div className="card">
        ...
        <div
          className="w-full h-3 bg-dark-border rounded-full overflow-hidden"
          role="progressbar"
                ...
                <button
                  onClick={() => {
                    api
                      ...
                      .catch(() =>
                        toast("Nu s-a putut reporni jobul", "error"),
                      );
                  }}
          ...
          <div className="flex flex-wrap gap-2">
            {["anaf", "openapi", "bilant", "bnr", "seap"].map((source) => (
              <button
                ...
                onClick={() => {
                  api
                    .retrySource(id, source)
                    .then((res) => {
                      if (res.success) {
                        toast(
                          `Sursa ${source.toUpperCase()} reinterogata cu succes`,
                          "success",
                        );
                        setLogs((prev) => [
                          ...prev,
                          {
                            text: `Retry ${source}: OK`,
                            type: "success",
                        ...
                        toast(
                          `Eroare ${source}: ${res.error || "necunoscuta"}`,
                          "warning",
                        );
                        setLogs((prev) => [
                          ...prev,
                          {
                            text: `Retry ${source}: ${res.error || "eroare"}`,
                            type: "warning",
                            ts: new Date().toLocaleTimeString("ro-RO", {
                              hour: "2-digit",
                              minute: "2-digit",
                    ...
                    .catch(() =>
                      toast(
                        `Eroare la reinterogarea sursei ${source}`,
                        "error",
                      ),
                    );
                }}
      ...
      <div className="card" aria-live="polite">
        ...
        {logs.length === 0 ? (
          <p className="text-xs text-gray-600">Nicio activitate inca...</p>
        ) : (
          <div className="space-y-1.5 max-h-60 overflow-y-auto font-mono">
            {logs.map((log, i) => (
              <div key={i} className="flex items-start gap-2 text-xs">
                ...
                {log.type === "info" && (
                  <span className="w-3.5 h-3.5 shrink-0 mt-0.5 text-blue-400 font-bold text-[10px] flex items-center justify-center">
                    ▸
                  </span>
                )}
                <span
        ...
        {job.status === "PENDING" && (
          <button
            ...
            onClick={() =>
              api
                .startJob(job.id)
                .then(() => setJob({ ...job, status: "RUNNING" }))
            }
          >
            Porneste Analiza
          </button>
