...
const FIELD_CONFIG: {
...
}[] = [
  {
    ...
    type: "password",
    group: "api",
    hint: "Web search (1000 req/luna gratuit)",
  },
  {
    ...
    type: "password",
    group: "api",
    hint: "Fallback AI autonom (gratuit)",
  },
  {
    key: "SYNTHESIS_MODE",
    label: "Mod Synthesis",
...
export default function Settings() {
  ...
  const [integrationTests, setIntegrationTests] = useState<
    Record<string, { status: "loading" | "ok" | "fail"; message?: string }>
  >({});
    ...
    try {
      ...
      if (status && status.status === "ok") {
        setIntegrationTests((prev) => ({
          ...prev,
          [name]: { status: "ok", message: "Conectat" },
        }));
      } else if (status) {
        const errMsg =
        ...
        setIntegrationTests((prev) => ({
          ...prev,
          [name]: { status: "fail", message: errMsg },
        }));
      } else {
        ...
        if (name === "api" && dbStatus?.status === "ok") {
          setIntegrationTests((prev) => ({
            ...prev,
            [name]: { status: "ok", message: "Backend OK" },
          }));
        } else {
          setIntegrationTests((prev) => ({
            ...prev,
            [name]: { status: "fail", message: "Nedetectat in health" },
          }));
        }
      }
    } catch {
      setIntegrationTests((prev) => ({
        ...prev,
        [name]: { status: "fail", message: "Eroare conexiune" },
      }));
    }
  };
  ...
  const handleTestService = async (service: string) => {
    ...
    try {
      ...
      setServiceTests((prev) => ({
        ...prev,
        [service]: { ok: false, message: "Eroare conexiune" },
      }));
    }
  };
  ...
  const renderGroup = (
    ...
    <div className="card space-y-4">
      ...
      {FIELD_CONFIG.filter((f) => f.group === groupKey).map((field) => {
        ...
        const svcResult = testServiceKey
          ? serviceTests[testServiceKey]
          : undefined;
        return (
          <div key={field.key}>
              ...
              svcResult !== null && (
                <p
                  className={clsx(
                    ...
                    svcResult.ok ? "text-green-400" : "text-red-400",
                  )}
                >
                  {svcResult.ok ? (
      ...
      <div className="flex items-center gap-3 pt-2 border-t border-dark-border/50">
        ...
        {integrationTests[groupKey]?.status === "ok" && (
          <span className="flex items-center gap-1 text-xs text-green-400">
            ...
            {integrationTests[groupKey].message}
          </span>
        )}
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      ...
      {isNotificationSupported() && (
        <div className="card space-y-3">
          ...
          <p className="text-xs text-gray-500">
            Primeste notificari native cand o analiza se finalizeaza, chiar daca
            tab-ul e minimizat.
          </p>
      ...
      {/* G5: Diagnostice + Log Viewer */}
      <div className="card space-y-4">
        <h3 className="text-sm font-semibold text-gray-400 uppercase">
          Diagnostice — Log Viewer
        </h3>
        ...
        <p className="text-xs text-gray-600">
          Logurile de interactiune (click-uri, actiuni, erori API) sunt salvate
          automat in{" "}
          <code className="font-mono text-gray-500">logs/ris_frontend.log</code>
          .
        </p>
      </div>
    </div>
...
function LogViewerButton() {
  ...
  const loadLogs = async () => {
    setLoading(true);
    try {
      const res = await api.getFrontendLogs(300);
      setLogs(res.lines ?? []);
    } catch {
      toast("Nu s-au putut incarca log-urile", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  const levelColor = (line: string) => {
    if (
      line.includes("] ERROR   ") ||
      (line.includes("] CONSOLE ") && line.includes("ERROR"))
    )
      return "text-red-400";
    if (line.includes("] CLICK   ")) return "text-blue-400";
    if (line.includes("] ACTION  ")) return "text-green-400";
    if (line.includes("] API_FAIL")) return "text-orange-400";
    if (line.includes("] VALIDATE")) return "text-yellow-400";
    if (line.includes("===")) return "text-accent-secondary font-bold";
    ...
    <>
      <button
        onClick={() => {
          setOpen(true);
          loadLogs();
          logAction("Settings", "openLogViewer");
        }}
        className="btn-secondary flex items-center gap-2 text-sm"
      >
        <FlaskConical className="w-4 h-4" />
        Vizualizeaza log-uri
      </button>
      ...
      {open && (
        ...
        >
          <div
            className="bg-dark-card border border-dark-border rounded-xl w-full max-w-4xl max-h-[85vh] flex flex-col"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-center justify-between p-4 border-b border-dark-border">
              <h2 className="text-white font-semibold flex items-center gap-2">
                <FlaskConical className="w-4 h-4 text-accent-primary" />
                Log Viewer — ris_frontend.log ({logs.length} linii)
              </h2>
              <div className="flex items-center gap-2">
                <input
                  value={filter}
                  onChange={(e) => setFilter(e.target.value)}
                  placeholder="Filtru (ERROR, CLICK, ACTION...)"
                  className="bg-dark-surface border border-dark-border rounded px-2 py-1 text-xs text-gray-300 focus:outline-none focus:border-accent-primary w-48"
                />
                <button
                  onClick={loadLogs}
            ...
            <div className="overflow-y-auto flex-1 p-3 font-mono text-xs space-y-0.5">
              {loading ? (
                <p className="text-gray-500 text-center py-8">Se incarca...</p>
              ) : filteredLogs.length === 0 ? (
                <p className="text-gray-600 text-center py-8">
                  Nicio intrare gasita
                </p>
              ) : (
                [...filteredLogs].reverse().map((line, i) => (
                  <p
                    key={i}
                    className={`whitespace-pre-wrap break-all leading-5 ${levelColor(line)}`}
                  >
                    {line}
                  </p>
                ))
            ...
            <div className="p-3 border-t border-dark-border flex items-center gap-2 text-xs text-gray-600">
              <span>{filteredLogs.length} linii afisate</span>
              <span>·</span>
              <span>Cele mai recente sunt primele</span>
            </div>
          </div>
        </div>
      )}
