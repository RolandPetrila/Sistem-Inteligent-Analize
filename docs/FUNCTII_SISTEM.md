...
## Backend — 52+ REST Endpoints + 1 WebSocket
...
| POST   | /api/jobs/{id}/section/{key}/regenerate | Re-genereaza o singura sectiune dintr-un raport existent (F3-7) |
...
| GET    | /api/reports/{id}/data               | Full data JSON filtrat per sectiune (FIX #34)       |
| GET    | /api/reports/{id}/delta              | Delta modificari fata de analiza anterioara         |
| GET    | /api/reports/{id}/export/ics         | Export licitatii SEAP ca iCalendar (.ics)           |
| POST   | /api/reports/{id}/share              | Genereaza link partajabil HTML (token 30 zile) (B5) |
| GET    | /api/reports/public/{token}          | Acces public raport HTML via token (fara auth) (B5) |
...
| GET    | /api/monitoring/audit-log     | Log auditabil schimbari detectate (R6)                                     |
| POST   | /api/monitoring/{id}/suppress | Suprima alerta pentru N ore (R6)                                           |

### Predictive & Network API (R6)

| Metoda | Endpoint                       | Descriere                                                                |
| ------ | ------------------------------ | ------------------------------------------------------------------------ |
| GET    | /api/companies/{id}/predictive | Modele predictive: Altman Z-Score, Piotroski, Beneish M-Score, Zmijewski |
| GET    | /api/companies/{id}/network    | Retea firme: asociati comuni, administratori, relatii indirecte          |
| POST   | /api/quick-score               | Scor rapid dintr-un CUI fara analiza completa                            |
| GET    | /api/companies/{id}/funding    | Programe de finantare eligibile per profil firma                         |
...
- Sistem logging complet per job: logs/job\_{id}.log
...
- ZIP Auto-Pack: toate formatele generate impachetate automat in raport*{CUI}*{date}.zip
...
- C22: Settings phantom save — \_reload_settings() with object.**setattr**
...
## Functionalitati Faza R7 (2026-04-09 — Quick Wins + Features UX + Calitate)

### A — Quick Wins

- A1: Număr Raport Unic format RIS-YYYY-XXXX (auto-increment DB per an, cover PDF + header HTML)
- A2: Risk Badge numeric pe carduri Companies (afișaj `score/100` cu culoare Verde/Galben/Rosu + aria-label)
- A5: AEGRM Client — Garanții Reale Mobiliare (aegrm_client.py, penalizare scoring juridic -5 dacă găsite)

### B — Features UX

- B1: NLQ Ask RIS Chatbot (5 intenții: top_risc/statistici/ultimele/firma_info/comparatie, chat panel flotant)
- B2: Knowledge Graph Visualizer (/network/:cui, React Flow, noduri colorate, relaţii BFS depth-4)
- B3: Dark/Light Theme Toggle (ThemeToggle în sidebar, persistenţă localStorage, initTheme startup)
- B4: GlobalSearch pe Mobile (buton lupă în mobile header, dispatch Ctrl+K)
- B5: Link Partajabil Raport HTML (share_token unic, endpoint public fără auth, TTL 30 zile)

### C — Calitate Cod

- C1: Split fișiere frontend >500 LOC (ReportView, CompanyDetail, Dashboard, NewAnalysis → componente)
- C2: TanStack Query (@tanstack/react-query) — înlocuiește useEffect+useState pentru data fetching
- C3: Accessibility ARIA — aria-label pe icoane, role=alert Toast, text alternativ culori risc
- C4: Type hints pe funcţii publice backend (agent_official, cache_service, notification)

### E — Surse Noi

- E3: Mistral OCR (POST /api/documents/ocr — upload PDF scanat → text structurat via pixtral-12b)
...
| Retea Firme        | /network/:cui | Knowledge Graph vizual: asociati, administratori, relatii BFS (B2) |
...
| ZIP Auto-Pack | zipfile     | Toate formatele generate intr-un singur raport*{CUI}*{date}.zip (8C) |
...
## Noutati R6 (2026-04-07)

### Modele Predictive (Altman / Piotroski / Beneish / Zmijewski)

- Altman Z-Score: scor predictie faliment bazat pe 5 ratii financiare (working capital, RE, EBIT, equity, sales)
- Piotroski F-Score: 9 criterii binare de sanatate financiara (0-9 puncte)
- Beneish M-Score: detectie manipulare contabila (8 indicatori)
- Zmijewski Score: model logistic faliment (3 variabile: ROA, leverage, lichiditate)
- Endpoint: GET /api/companies/{id}/predictive

### Retea Firme

- Detectie asociati comuni intre firme (graph de relatii)
- Identificare administratori care apar la multiple companii
- Relatii indirecte (firma A -> persoana X -> firma B)
- Vizualizare graf retea in frontend
- Endpoint: GET /api/companies/{id}/network

### Fonduri Finantare

- Baza de date locala programe active (JSON local)
- Matching eligibilitate per profil: CAEN, angajati, vechime, regiune, datorii ANAF
- Sumar text generat automat cu top 3 programe eligibile
...
- Endpoint nou: POST /api/jobs/{id}/section/{key}/regenerate
- Re-genereaza doar o sectiune specifica (ex: swot, competition, recommendations)
- Nu reface intreaga analiza - economie timp + resurse AI
- Sectiuni valide: executive_summary, company_profile, financial_analysis, risk_assessment, market_position, opportunities, recommendations, swot, competition
