...
> **Gotcha critic:** default-ul `pydantic-settings` citește env var-urile ÎNAINTEA lui `.env`, DAR RIS **inversează** ordinea surselor (`backend/config.py::settings_customise_sources`, fix `739f52f`, regresie `tests/test_config_source_priority.py`) → în RIS **`.env` bate env var**; `.env` e sursa de adevăr pentru producție. **Un shell care citește env var-ul (sau pornit înainte de ultimul sync) poate diverge de ce folosește RIS** — ca să testezi ce vede producția, citește `.env`.
...
| `XAI_API_KEY`        | xAI Grok                | ⚠️     | **doar câmpul în config** — zero cod de integrare                                                                                                                                  |
...
| —                      | INS TEMPO                                                          | 🚫 nuanțat                 | Apelul RIS (`INT101B` pe `/tempo-online/rest/data/`) → **404** (măsurat 2026-07-27, GET); DAR serviciul e **VIU** pe alt base: `GET /tempo-ins/matrix/INT101W` → **200**. RIS lovește baza retrasă, nu cea vie. |
...
2. **`.env` RIS** (root proiect) — adaugi `NUME_CHEIE=valoare`. Fișierul e gitignored.
   ⚠️ **RIS inversează ordinea surselor** (`739f52f`) → producția citește **`.env`**, nu env var-ul tău. `.env` e sursa de adevăr pentru producție.
...
| **Benchmark sector** | `CAEN_BENCHMARK` static | INS TEMPO — apel RIS `INT101B` → **404** @2026-07-27 (base `/tempo-online/` retras; serviciul viu pe `/tempo-ins/INT101W`) | Eurostat ✅ · gol în **63%** din rapoarte                                         |
...
# ISTORIC: drift shell vs producție — rezolvat structural în 739f52f (.env bate env var în RIS; divergența nu mai contează pt producție)
